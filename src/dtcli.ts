...
const buffer = createReport({
  ...
  postProcessor: (rootNodeOrig, filename) => {
    ...
    if (! ["word/header1.xml", "word/header2.xml", "word/header3.xml"].includes(filename)) {
      return rootNodeOrig
    }
    
    // let's do a deepcopy that strips a w:r element that contains //> v:textpath string=DRAFT

    // and let's abuse Javascript's prototype inheritance model! ha, ha.
    ...
    let hasChildrenNamed = function (mythis: Node, wantedname:string) {
      if (mythis._children
        ...
        return mythis._children.filter((x) => ! x._fTextNode && x._tag == wantedname)
      }
      else { return [] }
    }
    
    // we want to match a w:r /> w:pict /> v:shape /> v:textpath hasAttrValue "string" "DRAFT"
    let rootNodeOut = deepCopyNode(rootNodeOrig, (n:Node) => ! n._fTextNode && n._tag=="w:r"
...
function deepCopyNode(orig:Node, undesirable:((n:Node)=>boolean)) : (TextNode | NonTextNode) {

  let newNode;
  let copy = { _ifName: orig._ifName, _fTextNode: orig._fTextNode, _children:orig._children.filter((x) => ! undesirable(x)).map((x) => deepCopyNode(x, undesirable)) }
  if (orig._fTextNode) { newNode =    <TextNode>{ _text: orig._text, ...copy } }
  else                 { newNode = <NonTextNode>{  _tag: orig._tag,  ...copy, _attrs: orig._attrs } }

  for (var child of newNode._children) { child._parent = newNode }
